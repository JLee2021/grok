...
async function setupAppCrumbs(el, { $prop = 'default' } = { $prop: 'default' } ) {
  console.info('Do something with passed in property: $prop')

  // Update Component
  async function update(el) {
    console.info('Updat AppCrumbs List')
    el.innerHTML = template

    // Add Actions
    // el.querySelector('#frag').innerHTML = $frag($ref.value || [])
    // el.querySelector('#action').addEventListener('click', toAppCrumbsOther)
  }
