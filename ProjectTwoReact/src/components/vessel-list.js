...
async function setupVesselList(el) {
  ...
  async function update(el) {
    ...
    // Add Actions
    el.querySelector('#vessel-list').innerHTML =
      listVessels(vessels.value || [])
    Array.from(el.querySelectorAll('.vessel'))
      .map(item => item.addEventListener('click', toTripList))
  }
...
function toTripList(el) {
  el.preventDefault()
  // Grab vessel permit no. form data-id attribute.
